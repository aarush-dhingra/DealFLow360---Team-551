...
export default function UserManagementPage() {
  ...
  async function remove(account: Account) { if (!confirm(`Delete ${account.display_name}? Their login will be permanently removed. Historical business records will remain.`)) return; try { await api.delete(`/admin/users/${account.id}`); load(); } catch (err: unknown) { setError(err instanceof Error ? err.message : 'Could not delete account.'); } }
  return <div className="max-w-5xl"><PageHeader title="Team access" subtitle="Create internal accounts with a temporary password. Each person must set their own password at first sign-in." />
    ...
    <Card><Table headers={['Name', 'Email', 'Role', 'First sign-in', '']} >{accounts.map((account) => <Tr key={account.id}><Td className="font-medium">{account.display_name}</Td><Td>{account.email}</Td><Td>{account.roles.join(', ').replaceAll('_', ' ')}</Td><Td>{account.must_change_password ? 'Password change required' : 'Active'}</Td><Td><button onClick={() => remove(account)} className="text-xs font-medium text-red-600 hover:text-red-800">Delete</button></Td></Tr>)}</Table></Card>
  </div>;
}
