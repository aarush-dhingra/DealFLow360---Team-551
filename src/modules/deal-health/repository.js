...
/**
 * Computes and stores a deal health assessment for a quotation.
 * All score arithmetic runs in PostgreSQL using the active policy values.
 */
export async function assessAndStore(quotationId, policy) {
  ...
  try {
    ...
    // Score computed in JS using policy values (all from NUMERIC DB fields, no user input)
    const score =
    ...
    const { rows: assessmentRows } = await client.query(
      ...
      [
        ...
        inactDays,
        score,
        'normal', // band will be determined by service
