...
import { NotFoundError, ForbiddenError, ConflictError, AppError } from '../../shared/errors.js';
...
async function validateApproverAction(approvalId, actorUser, requiredRole) {
  const approval = await repo.getApprovalById(approvalId);
  if (!approval) throw new NotFoundError('Approval');
  if (approval.status !== 'pending') {
    throw new ConflictError(`Approval is already ${approval.status}`);
  }
  if (!actorUser.roles.includes(requiredRole) && !actorUser.roles.includes('admin')) {
    throw new ForbiddenError(`This approval requires the ${requiredRole} role`);
  }

  // Fetch current quotation lock version
...
export async function approveQuotation(approvalId, actorUser, reason) {
  const { approval, quotation } = await validateApproverAction(
    approvalId, actorUser, approval?.required_role ?? 'sales_manager'
  );
  ...
  try {
    ...
    // If manager approved a manager_then_finance route, create Finance step
    if (
      approval.required_role === 'sales_manager' &&
      policy.high_risk_route === 'manager_then_finance'
    ) {
      // Check if the risk actually requires finance (blended risk > manager max)
...
export async function rejectQuotation(approvalId, actorUser, reason) {
  ...
  try {
    ...
    // Cancel any other pending approval instances for this version
    await client.query(
...
export async function escalateToFinance(approvalId, actorUser, reason) {
  const { approval, quotation } = await validateApproverAction(
    approvalId, actorUser, 'sales_manager'
  );
