...
export async function requireAuthentication(request, _response, next) {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT s.id AS session_id, u.id, u.email, u.display_name, u.is_active, u.must_change_password, array_agg(ur.role) AS roles
...
export function requireRole(...roles) {
  return (request, _response, next) => {
    ...
    if (request.principal.must_change_password) return next(new AppError(403, 'PASSWORD_CHANGE_REQUIRED', 'You must change your temporary password before continuing.'));
