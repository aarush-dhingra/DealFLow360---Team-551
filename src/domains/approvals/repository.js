...
export async function upsertApprovalPolicy(managerMax, highRiskRoute, actorUserId) {
  ...
  try {
    ...
    await client.query(`UPDATE approval_policies SET is_active = false WHERE is_active = true`);
...
export async function getApprovalWithFullDetail(id) {
  ...
  if (approval.risk_assessment_id) {
    ...
    if (approval.risk) {
      ...
      approval.risk.flagged_lines = lineRows.filter(l => parseFloat(l.line_overage_percent) > 0);
