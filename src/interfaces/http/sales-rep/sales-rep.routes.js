...
quoteRouter.get('/meta/products', async (_req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT p.id, p.name, p.sku, p.list_price, p.unit_name, p.billing_kind, c.code AS category_code, c.discount_ceiling_percent FROM products p JOIN product_categories c ON c.id = p.category_id WHERE p.is_active ORDER BY p.name`
    );
    res.json({ data: rows });
  } catch (err) { next(err); }
});
